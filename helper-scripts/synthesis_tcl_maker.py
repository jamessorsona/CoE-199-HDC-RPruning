...
def create_tcl_file(tcl_file, filename, raw_filename):
    with open(tcl_file, "a") as fh:
        ...
        # fh.write(f"write_file -format verilog -hierarchy -output mapped/{filename}/{filename}_mapped.v\n")
...
def execute_tcl_file(tcl_file, filename):
...
def main():
    ...
    execute_tcl_file(tcl_file, filename)
    print("-----------------------------------------------")
    print("DONE!")
